...
- **WHEN** `Twin._invoke_llm()` calls the provider and receives a `ProviderTimeoutError` on the first attempt, but succeeds on the second attempt
...
- **WHEN** `Twin._invoke_llm()` calls the provider and receives `ProviderTimeoutError` on all attempts (1 initial + `timeout_retries` retries)
...
- **WHEN** `Twin._extract_self_audit_llm()` calls the provider and receives a `ProviderTimeoutError` on the first attempt, but succeeds on the second attempt
...
- **WHEN** `Twin._extract_self_audit_llm()` calls the provider and receives `ProviderTimeoutError` on all attempts
...
- **THEN** `invoke_with_timeout_retry` SHALL invoke the provider once and let any `ProviderTimeoutError` propagate immediately without retry
